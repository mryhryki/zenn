...
しかし、5GBを超えるファイルを単純にアップロードする事ができません。
マルチパートアップロードを使う必要があります。
...
例えば、複雑な条件（プレフィックス、容量、作成日時、`Content-Type` など）を判定して対象のファイルだけコピーしたい、などのケースも比較的容易に実装できます。
（シェルスクリプトで AWS CLI と `jq` などを駆使してやることもできなくはないですが、色々辛いですし、ローカルの容量に影響を受けて実行中にエラーになって絶望したこともあります）
