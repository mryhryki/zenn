...
AWS 初期からあるのも納得の便利サービスで、私もAWSでよく使うサービスの一つです。
しかしながら、5GBを超えるファイルをアップロードする場合は、単純にアップロードする事ができず、マルチパートアップロードを使う必要があります。

また S3 は便利なサービスなので、S3 互換をうたうサービスも多く存在しており、私は個人的に [Wasabi](https://wasabi.com/) というサービスも併用しています。
ただサービス自体が異なる場合は、バケット間のデータ移動、特に大容量のデータの場合は結構面倒です。

そこで今回は、マルチパートアップロードを理解して、異なる S3 サービス間でも効率的にコピーする方法を紹介しようと思います。
...
私の場合 `STANDARD` と `GLACIER DEEP ARCHIVE` の２つをよく使っています。
...
保管料金が特に安いので、ほぼ取り出す可能性のない大容量の定期バックアップデータとかを入れています。
取り出しに時間がかかることや、大容量のデータをダウンロードするとそれなりにインターネット転送料がかかる点に注意です。
...
ただ実際にどの程度速くできそうかは検証していないので、機会があれば検証して追記したいと思います。

### ローカルを経由する場合、空き容量に制限されない

これは異なる S3 サービス間でファイルを移動する場合など、一度ローカルを経由するようなケースでのメリットで、今回の重要な目的の一つです。
大容量ファイルをコピーする場合、AWS CLI を使うケースなどであれば一度ローカルにダウンロードしてアップロードすると思いますが、最小で5MBごとにダウンロード→アップロードすることで、ローカルの保存容量を最小限に済ませることができます。
...
マルチパートアップロードは3〜4種類の API を使ってアップロードをする必要があるので、どうしても手順が複雑になります。
...
私は、消し忘れを防ぐため `2.` を設定しつつ、なるべく保管料金抑えるために `1.` で削除するようにしています。



# S3 間でコピーする４つの方法

S3 間でのコピーする方法は大きく分けて以下の４パターンがあります。
...
`4` の方法を使えば、異なるS3サービス間でも大容量ファイルを効率良くコピーすることができます。
...
AWS SDK を使いプログラムから大容量のファイルを扱えるようになると非常に便利になりました。
例えば、一定の条件（容量や作成日など）を満たした場合にバケットを移動するなど、ちょっと複雑な条件であっても容易にプログラミングすることができます。

S3 をよく使うエンジニアであれば、是非マルチパートアップロードも使いこなせると、用途の幅も広がると思います。
